...
public class RoboThieves {
       ...
       ArrayList<Position>cp = new ArrayList();
        for(int i=0;i<row;i++){
            ...
            for(int j=0;j<col;j++){
                ...
                c.y=j;
                switch(graph[i][j]){
                    case 'W': NumGraph[i][j] = -1;break;
                    case 'C': NumGraph[i][j] = -2;cp.add(c);break;
                    case 'S': NumGraph[i][j] = 0; sp[0] = i;sp[1] = j;break;
                    default:
                        NumGraph[i][j] = -3;break;
                }
            }
        }
    ...
    private static void step(int i, int j, ArrayList<Position> nextlist, Position pos,int s) {
        ...
        pos1.x = pos.x+i;
        pos1.y = pos.y+j;
        if(!allow(pos1.x,pos1.y)) return;
        if(NumGraph[pos1.x][pos1.y]==-2)return;
            ...
            for(int j=0;j<col;j++){
                if(graph[i][j]=='.'){
                    if(NumGraph[i][j]<0)
                        ...
                        System.out.println(NumGraph[i][j]);
                }
            }
        }
    }
